...
const formSchema = z
  ...
  .refine((data) => data.price >= data.deviceDiscount, {
    message: "La réduction doit être inferieure ou égale au prix de la console",
    path: ["deviceDiscount"],
  });
...
const CategoryForm = (props: CategoryFormProps) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast.success(
        `${props.category ? "Modification" : "Création"} effectuée avec succès`
      );
    } catch (error) {
      toast.error(`${error}`);
    }
    ...
    <Form {...form}>
      ...
      >
        ...
        <div className="flex flex-row gap-4 justify-end">
          ...
          <Button type="submit">
            {props.category ? "Modifier la console" : "Créer une console"}
          </Button>
        </div>
      </form>
    </Form>
