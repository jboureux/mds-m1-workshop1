...
const ConsolePreview = (props: ConsolePreviewProps) => {
  ...
  const getBase = () => {
    const baseAccessory = props.selectedOptions.find((option) => option.isBase);

    const baseImage =
      baseAccessory && baseAccessory.variant && baseAccessory.variant.images
        ? baseAccessory.variant.images.find((image) => props.view in image)
        : undefined;
  ...
  const getLayers = (): string[] => {
    const noBaseLayers = props.selectedOptions.filter(
      (option) => option.variant && option.variant.images && !option.isBase
    );
